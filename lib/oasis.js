...
export default class Oasis {
  ...
  rehydrateCard(element) {
    ...
    let style = card.element.style;
    style.display = 'none';
    style.position = 'absolute';
    style.top = (rect.top - offset.top) + 'px';
    style.left = (rect.left - offset.left) + 'px';
    style.right = (offset.right - rect.right) + 'px';
    style.bottom = (offset.bottom - rect.bottom) + 'px';
    style.transformOrigin = (-1 * rect.left) + 'px ' + (-1 * rect.top) + 'px';
    style.zIndex = 2000000;
    document.body.insertBefore(card.element, document.body.firstChild);
  ...
  _findPeers() {
    ...
    let topFrames = window.parent.frames;

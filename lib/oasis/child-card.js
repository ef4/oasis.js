...
export default class ChildCard extends BaseCard {
  ...
  positionOver(targetElement, containerElement) {
    ...
    this.positionTracker = {
      target: $target,       // Underlying element we track
      initialized: false,
      container: $container, // Our parent element
      absolute: null         // Temporary absolute positioning
                             // that overrides our tracking.
    };
    ...
    this.onDefault('setPreferredSize', request => {
      this.loaded().then(() => {
        if (request.position === 'absolute') {
          this.positionTracker.absolute = {
            height: request.height == null ? $target.height() : request.height,
            width: request.width == null ? $target.width() : request.width
          };
        } else {
          this.positionTracker.absolute = null;
  ...
  _trackPosition() {
    ...
    let width, height;
    if (tracker.absolute) {
      width = tracker.absolute.width;
      height = tracker.absolute.height;
    } else {
      width = tracker.target.width();
      height = tracker.target.height();
    }

    $elt.css({
      transform: `translateX(${(targetRect.left - containerRect.left)/t.a  - m.left}px) translateY(${(targetRect.top - containerRect.top)/t.d - m.top}px)`,
      transformOrigin: '0 0',

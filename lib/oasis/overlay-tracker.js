...
export default class OverlayTracker {
  ...
  _position() {
    ...
    return {
      top: targetRect.top - ownRect.top - this.margins.top + t.ty,
      left: targetRect.left - ownRect.left  - this.margins.left + t.tx
    };
  }

  _trackPosition(timer) {
    // if our timer is running slower than 30hz, we just slow down the motion, rather than let our step size get too big.
